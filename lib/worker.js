...
function Worker(options) {
  ...
  if (options.forwardMetadata === true) {
    this.forwardMetadata = true;
  } else {
    this.forwardMetadata = false;
  }
...
Worker.prototype._processFile = function(file, done) {
  ...
  function moveTx(file, fromFolder, toFolder, callback) {
    FS.rename(Path.join(fromFolder, file), Path.join(toFolder, file), function(err) {
      ...
      if (self.forwardMetadata) {
        ...
        FS.rename(Path.join(fromFolder, metadataFile), Path.join(toFolder, metadataFile), callback);
      } else {
        callback();
      }
    });
  ...
  function delTx(file, fromFolder, callback) {
    FS.unlink(Path.join(fromFolder, file), function(err) {
      ...
      if (self.forwardMetadata) {
        var metadataFile = Utils.getMetadataFilename(file);
        FS.unlink(Path.join(fromFolder, metadataFile), callback);
      } else {
        callback();
      }
    });
  ...
  function fetchMetadata(file, fromFolder) {
    var metadata = FS.readFileSync(Path.join(fromFolder, Utils.getMetadataFilename(file)));
  ...
  function handleError(err) {
    ...
    return moveTx(file, self.workingPath, self.errorPath, finish);
  }

  // Move to the working directory
  moveTx(file, this.queuePath, this.workingPath, function(err) {
    if (err) {
    ...
    var options;
    if (self.forwardMetadata) {
      options = fetchMetadata(file, self.workingPath);
    ...
    Needle.request(options.method, options.url, data, options, function(err, res, body) {
      ...
      delTx(file, self.workingPath, finish);
    });
  });
};
