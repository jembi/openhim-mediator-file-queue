...
function updateEndpointConfig(config, apiOpts, newConfig){
  Winston.info('Updated Endpoint config');
  config.set('endpoints', parseJSONIntoArray(newConfig));
  Winston.info(JSON.stringify(config.get('endpoints')));
...
function displayEndpoints(){
  Winston.info('File Queue Endpoints:')
    ...
    MUtils.registerMediator(apiOpts, mediatorConf, function(err) {
      ...
      MUtils.fetchConfig(apiOpts, (err, initialConfig) => {
        if (err) {
          ...
          Winston.info(JSON.stringify(initialConfig));
          mediatorConf.config = initialConfig
          updateEndpointConfig(config, apiOpts, initialConfig);
          ...
          app.listen(port, function() {
            ...
            heartbeat.on('config', function(newConfig){
              ...
              updateEndpointConfig(config, apiOpts, newConfig);
              displayEndpoints()
            });
